...
3. **Deploy builders**: For each wave from init-phase output, use `TaskCreate` with wave dependencies via `addBlockedBy`. Deploy builders using the **Task tool call** (never Bash/CLI) with `subagent_type="devorch-builder"` as **foreground parallel** calls following the wave structure.
   ...
   - For `"validation"` type waves: handled in step 5 below (not here).
...
4. **Run validation commands**: Run `bun $CLAUDE_HOME/devorch-scripts/run-validation.ts --plan .devorch/plans/current.md --phase N`

   Parse JSON output:
   - If all commands pass (`failed === 0`): proceed to step 5 with no validation issues.
   - If any command fails: include the failure output in the validator's prompt context (step 5) so it can assess whether it's a real problem.

5. **Deploy validator**: Deploy validator in foreground (use the **Task tool call** with `subagent_type="devorch-validator"`). Its prompt includes inline: phase **criteria** (from `<criteria>`), task summaries, relevant conventions, and any validation command failures from step 4. The validator focuses solely on **acceptance criteria verification** by reading code — it does not run validation commands (step 4 handles that). If FAIL → stop and report.

6. **Phase commit**: Run `git -C <projectRoot> status --porcelain`. If output is empty, skip commit. If output has changes:
...
7. **Invalidate and update cache**: Run `bun $CLAUDE_HOME/devorch-scripts/manage-cache.ts --action invalidate,trim --max-lines 3000 --root <mainRoot>`
...
8. **Update state**: Run `bun $CLAUDE_HOME/devorch-scripts/update-state.ts --plan .devorch/plans/current.md --phase N --status "ready for phase $((N+1))" --summary "<concise phase summary>"`
...
- Always update state.md (step 8), even on partial failure.
