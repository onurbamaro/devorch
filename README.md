...
| `/devorch:make-plan` | Creates phased plan with wave structure. Auto-generates CONVENTIONS.md on first run. | Explore |
...
commands:
  ...
  - name: make-plan
    ...
    purpose: Create phased plan with wave structure. Auto-generates CONVENTIONS.md
    flags: [--team]
  - name: build
...
state_files:
  - path: .devorch/PROJECT.md
    purpose: Product context from /new-idea (optional)
...
file_structure:
  source:
    - commands/ (10 .md slash command definitions)
